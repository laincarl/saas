...
import {
  Button, Tabs, Icon, Select, Table, Tooltip, 
} from 'choerodon-ui';
import {
...
class MergeRequestHome extends Component {
  ...
  render() {
    const {
      type, organizationId, name, id: projectId, 
    } = AppState.currentMenuType;
    const { MergeRequestStore, intl, history: { location: { state } } } = this.props;
    const { param, tabKey } = this.state;
    const {
      getPageInfo: {
        opened: op, merged: mp, closed: cp, all: ap, 
      },
      getMerge: {
        opened: od, merged: md, closed: cd, all: ad, 
      },
      getCount: {
...
!
      ...
      key: 'title',
      render: (text, record) => (
        <MouserOverWrapper text={record.title} width={0.2}>
          {record.title}
        </MouserOverWrapper>
      ),
    }, {
...
!
      ...
      key: 'title',
      render: (text, record) => (
        <MouserOverWrapper text={record.title} width={0.25}>
          {record.title}
        </MouserOverWrapper>
      ),
    }, {
