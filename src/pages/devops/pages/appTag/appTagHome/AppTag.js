...
import {
  Button, Select, Modal, Form, Icon, Collapse, Avatar, Pagination, Tooltip, 
} from 'choerodon-ui';
...
class AppTag extends Component {
  ...
  render() {
    ...
    const {
      visible, deleteLoading, creationDisplay, appName, editDisplay, editTag, editRelease, tag, 
    } = this.state;
    ...
    return (
      ...
      >
        {appData && appData.length && appId ? (
          <Fragment>
            ...
            <Content code={appData.length ? 'apptag.app' : 'apptag'} values={{ name: titleName }}>
              ...
              {loading || _.isNull(loading) ? <LoadingBar display /> : (
                <Fragment>
                  {tagList.length ? (
                    <Fragment>
        ...
        ) : <DepPipelineEmpty title={<FormattedMessage id="apptag.head" />} type="app" />}
      </Page>
    );
  }
}
